...
Shows:
  - Today's slate scores (all four targets) with Statcast context
  - Season Statcast quality-of-contact metrics (Barrel%, HH%, AvgEV, xBA)
  - Rolling 30-day pitch-level metrics (when pybaseball available)
  - Matchup breakdown vs today's pitcher

Called from app.py as: player_profile_page(df, player_id_map, filters)
  df            — fully scored slate DataFrame
  player_id_map — dict[batter_name → MLBAM player ID] from mlb_api
  filters       — dict with use_gc, score_col keys (for context only)
"""
...
def _score_bar(score: float, color: str) -> str:
    ...
    return (
        ...
        f'    <div style="width:{pct}%;background:{color};height:10px;border-radius:4px;'
...
def _metric_row(label: str, value: str, delta: str = "", good: bool = None) -> str:
    delta_color = (
        "color:#4ade80" if good is True  else
        "color:#f87171" if good is False else
        "color:#94a3b8"
    )
    delta_html = f'<span style="font-size:.72rem;{delta_color};margin-left:.4rem">{delta}</span>' if delta else ""
    return (
...
def player_profile_page(df: pd.DataFrame, player_id_map: dict, filters: dict):
    ...
    <div style="background:#0f1923;border:1px solid #1e2d3d;border-radius:10px;
      ...
      <div style="font-size:.85rem;color:#94a3b8;margin-top:.4rem">
        ...
        &nbsp;·&nbsp; Pitcher: <b style="color:#e2e8f0">{pitch_grd}</b>
      </div>
    </div>
    ...
    for i, (sc, label, color) in enumerate(score_targets):
        gc_sc   = sc + '_gc'
        display = sc if not use_gc or gc_sc not in df.columns else gc_sc
        score   = float(row.get(display, 0))
        with cols[i]:
            st.markdown(
    ...
    # ── Two-column layout: Matchup + Statcast ─────────────────────────────────
    ...
    with left:
        ...
        LG = CONFIG
        k_vs_lg   = LG['league_k_avg']  - _s('p_k')
        hr_vs_lg  = _s('p_hr')          - LG['league_hr_avg']
        bb_vs_lg  = LG['league_bb_avg'] - _s('p_bb')

        items = [
            ("Hit Prob (1B+XB+HR)",  f"{_s('total_hit_prob'):.1f}%",   "", None),
            ("1B Prob",              f"{_s('p_1b'):.1f}%",              "", None),
            ("XB Prob",              f"{_s('p_xb'):.1f}%",              "", None),
            ("HR Prob",              f"{_s('p_hr'):.1f}%",              f"vs lg: {hr_vs_lg:+.2f}%",  hr_vs_lg  >= 0),
            ("K Prob",               f"{_s('p_k'):.1f}%",               f"vs lg: {k_vs_lg:+.1f}%",   k_vs_lg   >= 0),
            ("BB Prob",              f"{_s('p_bb'):.1f}%",              f"vs lg: {bb_vs_lg:+.1f}%",  bb_vs_lg  >= 0),
        ...
        # Historical (season vs this pitcher type)
        pa  = _s('PA')
        h   = _s('H')
        avg = _s('AVG')
        if pa >= 1:
            st.markdown("**Batter History (vs pitcher type)**")
            st.markdown(
                f'<div style="background:#0f1923;border:1px solid #1e2d3d;'
    ...
    with right:
        ...
        # Season aggregate (from FanGraphs via pybaseball batting_stats)
        sc_barrel = row.get('Barrel%', None)
        sc_hh     = row.get('HH%',     None)
        sc_avgev  = row.get('AvgEV',   None)
        sc_maxev  = row.get('maxEV',   None)
        sc_xba    = row.get('xBA',     None)
        sc_xslg   = row.get('xSLG',   None)
        sc_obp    = row.get('OBP',     None)
        sc_slg    = row.get('SLG',     None)

        has_season = any(
            pd.notna(v) for v in [sc_barrel, sc_hh, sc_avgev, sc_xba]
        )

        if has_season:
            LG_BARREL = 7.5
            LG_HH     = 38.0
            barrel_val = float(sc_barrel) if pd.notna(sc_barrel) else None
            hh_val     = float(sc_hh)     if pd.notna(sc_hh)     else None

            season_items = [
                ("Barrel%",  _fmt(sc_barrel, ".1f") + "%" if pd.notna(sc_barrel) else "—",
                 f"lg avg {LG_BARREL}%",  barrel_val >= LG_BARREL if barrel_val else None),
                ("HardHit%", _fmt(sc_hh, ".1f") + "%" if pd.notna(sc_hh) else "—",
                 f"lg avg {LG_HH}%",      hh_val    >= LG_HH     if hh_val    else None),
                ("Avg EV",   _fmt(sc_avgev,   ".1f") + " mph" if pd.notna(sc_avgev) else "—", "", None),
                ("Max EV",   _fmt(sc_maxev,   ".1f") + " mph" if pd.notna(sc_maxev) else "—", "", None),
            ...
            html2 += '</div>'
            st.markdown("**Season (FanGraphs)**")
            st.markdown(html2, unsafe_allow_html=True)
        else:
            st.info("Season Statcast data not available for this player.")

        # Rolling 30-day pitch-level (requires pybaseball + MLBAM ID)
        ...
        if player_id:
            ...
            st.info("MLBAM ID not found — rolling data unavailable. Check mlb_api player map.")

    # ── GC Context for today's game ───────────────────────────────────────────
